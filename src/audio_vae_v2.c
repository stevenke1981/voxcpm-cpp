...
struct ggml_tensor * vcpm_vae_residual_unit(struct ggml_context * ctx,
    ...
    if (a0) h = vcpm_vae_snake_activation(ctx, h, a0);
    if (dbg_slot + 0 < 32)
        g_dbg_tensors[dbg_slot + 0] = h;  /* after snake1 */
    if (g_dbg_capture_ru) vcpm_vae_save_snapshot(ctx, graph, h);
    ...
    if (a2) h = vcpm_vae_snake_activation(ctx, h, a2);
    if (dbg_slot + 2 < 32)
        g_dbg_tensors[dbg_slot + 2] = h;  /* after snake2 */
    if (g_dbg_capture_ru) vcpm_vae_save_snapshot(ctx, graph, h);

    /* Conv2: pointwise kernel=1, padding=0, dilation=1 */
    struct ggml_tensor * c2 = vcpm_vae_conv1d_layer(ctx, graph, conv2_w, conv2_b, h,
                                            1, 0, 1, model);
    if (!c2) return residual;
    if (dbg_slot + 3 < 32)
        g_dbg_tensors[dbg_slot + 3] = c2;  /* after pointwise conv (before skip) */
    if (g_dbg_capture_ru) vcpm_vae_save_snapshot(ctx, graph, c2);
    ...
    if (g_dbg_capture_ru) vcpm_vae_save_snapshot(ctx, graph, out);
    return out;
}
