...
static int vcpm_parse_args(int argc, char ** argv, int scan_offset,
                            vcpm_arg_def * defs, int n_defs) {
    int missing = 0;
    int flag_count = 0;
    ...
    for (int i = scan_offset; i < argc; i++) {
        ...
        if (eq) {
            ...
            if (len >= sizeof(arg_name)) len = sizeof(arg_name) - 1;
        ...
        for (int d = 0; d < n_defs; d++) {
            if (strcmp(arg_name, defs[d].name) != 0) continue;
    ...
    for (int d = 0; d < n_defs; d++) {
        ...
        switch (defs[d].type) {
            case VCPM_ARG_STRING:
                ...
                break;
            case VCPM_ARG_INT:
                if (defs[d].default_str) is_set = 1;
                else is_set = (*(int *)defs[d].dest != 0);
                break;
            case VCPM_ARG_FLOAT:
                if (defs[d].default_str) is_set = 1;
                else is_set = (*(float *)defs[d].dest != 0.0f);
                break;
            case VCPM_ARG_FLAG:
...
static int vcpm_stream_sink_cb(const float * samples, size_t n_samples,
...
static void apply_model_params(vcpm_model_params * mp,
    ...
    if (threads > 0) mp->n_threads = threads;
}
...
static int cmd_inspect(int argc, char ** argv) {
    ...
    if (vcpm_parse_args(argc, argv, 2, args, n_args) < 0) return 2;
...
static int do_tts_common(int argc, char ** argv, int streaming) {
    ...
    if (text_file) {
        ...
        if (!owned_text) { fclose(f); return 2; }
    ...
    if (streaming) {
        ...
        int write_ret;
        if (use_pcm16) {
            write_ret = vcpm_write_wav_pcm16(out_path, audio.samples,
                                             audio.sample_rate,
                                             audio.n_channels, audio.n_samples);
        } else {
            write_ret = vcpm_write_wav_f32(out_path, audio.samples,
                                           audio.sample_rate,
                                           audio.n_channels, audio.n_samples);
        }
        if (write_ret != 0) {
